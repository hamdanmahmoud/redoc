  ...
  return (
    <>
      <div style={{ display: 'flex' }}>
        <Input
          key={`arr-input-element-key-${index}`}
          borderColor={isEmpty(error) ? 'initial' : 'red'}
          margin={'0.5rem 0.75rem 0.5rem 0'}
          placeholder={'key'}
          disabled={saved}
          color={saved ? 'lightgrey' : '#1e1e1e'}
          onChange={e => handleOnChange('key', e)}
        />
        <Input
          key={`arr-input-element-value-${index}`}
        ...
        <RowIcon
          ...
          marginLeft={'0.5rem'}
          onClick={saved ? handleRemoveRow : f => f}
        >
          X
        </RowIcon>
      </div>
      {!isEmpty(error) && <div style={{ color: 'red', fontSize: 'smaller' }}>{error}</div>}
    </>
  );
};
...
export const DictionaryForm = ({ onChange, ancestors }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {array}
      <div style={{ ...addButtonStyle, color: '#21608a' }} onClick={handleAddRow}>
        <ActionOnArrayButton disabled={false}>{`+`}</ActionOnArrayButton>
        <div>{'Add pair'}</div>
      </div>
    </div>
  );
